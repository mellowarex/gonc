...
import (
	...
	"github.com/mellowarex/gonc/logger/colors"
)
...
// GonLogger logs logging records to the specified io.Writer
...
var Log = GetGonLogger(os.Stdout)
...
// GetGonLogger initializes the logger instance with a NewColorWriter output
// and returns a singleton
func GetGonLogger(w io.Writer) *GonLogger {
	once.Do(func() {
		var (
			err             error
		...
		instance = &GonLogger{output: colors.NewColorWriter(w)}
	})
	return instance
}

// SetOutput sets the logger output destination
func (l *GonLogger) SetOutput(w io.Writer) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *GonLogger) getLevelTag(level int) string {
	switch level {
	case levelFatal:
		return "FATAL   "
	case levelSuccess:
...
func (l *GonLogger) getColorLevel(level int) string {
	switch level {
	case levelCritical:
...
func (l *GonLogger) mustLog(level int, message string, args ...interface{}) {
	if level > logLevel {
		return
	}
	// Acquire the lock
...
func (l *GonLogger) mustLogDebug(message string, file string, line int, args ...interface{}) {
	if !debugMode {
		return
	}
