...
func NewWatcher(paths []string, files []string, isgenerate bool) {
	...
	if err != nil {
		gonLogger.Log.Fatalf("Failed to create watcher: %s", err)
	}
		...
		for {
			...
			case e := <-watcher.Events:
				...
				if t := eventTime[e.Name]; mt == t {
					gonLogger.Log.Hintf("Skipping: "+"%s", e.String())
					isBuild = false
				}
				...
				if isBuild {
					gonLogger.Log.Hintf("Event fired: %s", e)
					go func() {
			...
			case err := <-watcher.Errors:
				gonLogger.Log.Warnf("Watcher error: %s", err.Error()) // No need to exit here
			}
		}
	}()

	gonLogger.Log.Info("Initializing watcher...")
	for _, path := range paths {
		gonLogger.Log.Hintf("Watching: %s", path)
		err = watcher.Add(path)
		if err != nil {
			gonLogger.Log.Fatalf("Failed to watch directory: %s", err)
		}
	}
}
...
func AutoBuild(files []string, isgenerate bool) {
	...
	if isgenerate {
		gonLogger.Log.Info("Generating the docs...")
		icmd := exec.Command("gonc", "generate", "docs")
		icmd.Env = append(os.Environ(), "GOGC=off")
		err = icmd.Run()
		if err != nil {
			utils.Notify("", "Failed to generate the docs.")
			gonLogger.Log.Errorf("Failed to generate the docs.")
			return
		}
		gonLogger.Log.Success("Docs generated!")
	}
	appName := appname
	if err == nil {
		...
		if err != nil {
			...
			gonLogger.Log.Errorf("Failed to build the application: %s", stderr.String())
			return
		}
	}

	gonLogger.Log.Success("Built Successfully!")
	Restart(appName)
}
...
func Kill() {
	defer func() {
		if e := recover(); e != nil {
			gonLogger.Log.Infof("Kill recover: %s", e)
		}
	}()
	if cmd != nil && cmd.Process != nil {
		...
		case <-time.After(10 * time.Second):
			gonLogger.Log.Info("Timeout. Force kill cmd process")
			err := cmd.Process.Kill()
			if err != nil {
				gonLogger.Log.Errorf("Error while killing cmd process: %s", err)
			}
			return
		}
	}
...
func Restart(appname string) {
	gonLogger.Log.Debugf("Kill running process", utils.FILE(), utils.LINE())
	Kill()
	go Start(appname)
}
...
func Start(appname string) {
	gonLogger.Log.Infof("Restarting '%s'...", appname)
	...
	go cmd.Run()
	gonLogger.Log.Successf("'%s' is running...", appname)
	started <- true
}
...
func shouldIgnoreFile(filename string) bool {
	for _, regex := range ignoredFilesRegExps {
		...
		if err != nil {
			gonLogger.Log.Fatalf("Could not compile regular expression: %s", err)
		}
